...
function showAllMenu(obj) {
  ...
  for (let val in obj) {
    obj[val].forEach((objval) => {
      fragment += `<div style="width:180px;height:280px;display:flex;flex-direction:column;align-items:center;justify-content:center" class="menu-box">
        <img src="${objval.gambar}" style="width:170px;height:170px" class="icon">
        <h3>${objval.nama.length > 20 ? objval.nama.substring(0, 20) + '...' : objval.nama}</h3>
        <p>Rp.${objval.harga}</p>
        <div class="buttons">
        <button class="detail btn" data-id="${objval.id}" data-jenis="${objval.id_jenis}">Detail</button><a href="https://wa.me/082135555343?text=Saya mau beli nih...(${
        objval.nama
      }).Bisa share info lebih lanjut?" class="beli btn a" data-id="${objval.id}" data-jenis="${objval.id_jenis}">Beli</a>
        </div>       
    </div>`;
    });
  }
...
function showMakanan(arr) {
  ...
  arr.forEach((val) => {
    fragment += `<div style="width:180px;height:280px;display:flex;flex-direction:column;align-items:center;justify-content:center" class="menu-box">
          <img src="${val.gambar}" style="width:170px;height:170px" class="icon">
          <h3>${val.nama.length > 20 ? val.nama.substring(0, 20) + '...' : val.nama}</h3>
          <p>Rp.${val.harga}</p>
          <div class="buttons">
    ...
    data
      ...
      .then((result) => {
        let fragment = `<div class="hide-detail">
                ...
                <p class="text">Nama : <span class="highlight">${result.makanan[id].nama.toUpperCase()}</span></p>
                <p class="text">Harga : <span class="highlight-harga">Rp.${result.makanan[id].harga}</span></p>
    ...
    data
      ...
      .then((result) => {
        let fragment = `<div class="hide-detail">
                ...
                <p class="text">Nama : <span class="highlight">${result.minuman[id].nama.toUpperCase()}</span></p>
                <p class="text">Harga : <span class="highlight-harga">Rp.${result.minuman[id].harga}</span></p>
